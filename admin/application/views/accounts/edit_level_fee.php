<div class='form_content'>	
    <h3>Edit Level Fee Registration</h3>
    <form action="<?=site_url('accounts/edit_level_fee/'.$level_fee_id)?>" method='post'>
        <table>
            <tr>
                <th>Programme Name: <span class='req_mark'>*</span></th>
                <td><select name="programs_id" class="jprograms_id"><?=combo::registered_program_options($std_info['student_id'],$programs_id)?></select><?=form_error('programs_id','<span>','</span>')?></td>
            </tr>
            <tr>
                <th>Level Name <span class='req_mark'>*</span></th>
                <td><select name="levels_id" class="jlevels_list jaccount_level_id"><?=combo::get_program_levels($programs_id,$levels_id)?></select><?=form_error('levels_id','<span>','</span>')?></td>
            </tr>
            <tr>
                <th>Level Fee</th>
                <td><input type="text" name="level_fee" value="<?=$level_fee?>" class="text ui-widget-content ui-corner-all required width_160 jmapped_value" readonly /><?=form_error('level_fee','<span>','</span>')?></td>		</tr>
            <tr>
                <th>Level Extended Date</th>
                <td><input type="text" name="level_extd_date" value="<?=$level_extd_date?>" class="text ui-widget-content ui-corner-all required width_160 jextended_date date_picker" /><?=form_error('level_extd_date','<span>','</span>')?></td>
            </tr>
            <tr>
                <th>Extended Level Fee <span class='req_mark'>*</span></th>
                <td><input type="text" name="extended_fee" value="<?=$extended_fee?>" class="text ui-widget-content ui-corner-all required width_160 jextended_fee" /><span class="jextended_error"></span><?=form_error('extended_fee','<span>','</span>')?></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td><input type="text" name="total_balance" value="<?=$total_balance?>" class="text ui-widget-content ui-corner-all required width_160 jtotal_balance" autocomplete="off" /><span class="jbalance_error"></span><?=form_error('total_balance','<span>','</span>')?></td>
            </tr>
            <tr>
                <th>Comments</th>
                <td><textarea name="level_comment" rows="3" cols="30"><?=$level_comment?></textarea></td>
            </tr>
