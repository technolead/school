<div class='form_content'>
    <h3><?=$page_title?></h3>
    <form action='<?=site_url('staffs/edit_attendance/'.$attendance_id)?>' method='post'>
        <table>
            <tr>
                <th>Date <span class='req_mark'>*</span></th>
                <td><input type='text' name='date' value='<?=$date?>' class='input_txt width_160 date_picker' /><?=form_error('date','<span>','</span>')?></td>
            </tr>
            <tr>
                <th>Present</th>
            ...
            <tr>
                ...
                <td><input type='text' name='hours' value='<?=$hours?>' class='input_txt width_200' /><?=form_error('hours','<span>','</span>')?></td>
            </tr>
            <tr>
                <th>Extra Hours done <span class='req_mark'>*</span></th>
                <td><input type='text' name='extra_hour' value='<?=$extra_hour?>' class='input_txt width_200' /><?=form_error('semester_id','<span>','</span>')?></td>
            </tr>
             <tr>
                <th>Absent Hours <span class='req_mark'>*</span></th>
                <td><input type='text' name='absent_hour' value='<?=$absent_hour?>' class='input_txt width_200' /><?=form_error('semester_id','<span>','</span>')?></td>
            </tr>
            <tr>
                <th>Late <span class='req_mark'>*</span></th>
                <td><input type='text' name='late' value='<?=$late?>' class='input_txt width_200' /><?=form_error('semester_id','<span>','</span>')?></td>
            </tr>
			<tr>
                <th>Authorize <span class='req_mark'>*</span></th>
                <td><select name="authorize"><?=$this->mod_staffs->get_authorize(set_value('authorize',$authorize))?></select><?=form_error('authorize','<span>','</span>')?></td>
            </tr>
            <tr>
                <th>Comments</th>
