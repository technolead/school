    ...
    foreach($monthly_fee as $row): ?>
        <tr <?php if($inc%2==0) {
            ...
            <?
                ...
                $balance+=$payment+$row['total_fine'];
...
<div class='table_content'>
    ...
    foreach($staff_rows as $row):?>
        <tr <?php if($inc%2==0) {
            ...
            <td class="txt_right"><?php echo number_format($balance,2,'.',',')?></td>
        </tr>
        <?php $inc++;
    endforeach;
    ?>
...
<?php if(count($rows)>0 or count($agent_rows)>0 or count($staff_rows)>0) {
    echo'<div class="table_content">
				<table  cellpadding="0" cellspacing="0">
					...
					<tr class="odd">
						...
						<th style="text-align:right">'.number_format($std_credit+$staff_credit+$exp_credit,2,'.',',').'</th>
						<th style="text-align:right">'.number_format($std_debit+$staff_debit+$exp_debit,2,'.',',').'</th>
						<th style="text-align:right">'.number_format($balance,2,'.',',').'</th>
					</tr>
				</table>
		</div>';
