...
<div class='form_content'>
    <table cellpadding="1" cellspacing="1">
        ...
        <tr>
            <th>Fee Description : </th><td><?=$description?></td>
        </tr>
...
<div class='table_content'>
    ...
    $inc=1;
    foreach($rows as $row):
            $std_payment=common::get_additional_payment_info($row['student_id'],$additional_fee_register_id);
            if($row['is_delete']==0 || ($row['is_delete']==1 && count($std_payment)>0)){
        ?>
            <input type="hidden" name="additional_fee_id_<?=$row['student_id']?>" value="<?=$std_payment['additional_fee_id']?>" />
            <tr <?php if($inc%2==0) {
            echo "class='even'";
        }else {
            echo "class='odd'";
                ...
                <td><input type="text" name="fee_<?=$row['student_id']?>" value="<?=$std_payment['fee']?>" /></td>
    ...
    echo "<tr><td colspan='4'>No Content Found!!!</td></tr>";
        ...
        <div class="txt_center"><input type="hidden" value="<?=$register_date?>" name="date" /> <input type='submit' name='<?=$submit_name?>' value='<?=$submit_value?>' class='button' />
            <?if($submit_name=='update_additional_fee'){?><input type="button" name="delete" value="Delete Additional Fee" title="fee/delete_reg_additional_fee/<?=$additional_fee_register_id?>" class="jclass_fee_delete" ><?}?>
